  ...
  <body class="container">
    ...
    <form method="post" action="/orcamentos/{{ $orcamento-> id }}"> <!--chave primária do produta-->
        ...
        <div class="mb-3">
            ...
            <input type="text" id="nome" name="nome" value="{{ $orcamento->nome }}" class="form-control" disabled>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" id="preco" name="preco" value="{{ $orcamento->preco }}" class="form-control" disabled>
        </div>
        ...
        <div class="mb-3">
            ...
            <select id="categoria_id" name="categoria_id" class="form-select" disabled>
                @foreach ($categorias as $c)
                    <option value="{{ $c->id }}" {{ $orcamento->categoria_id == $c->id ? "selected" : "" }}> <!--operador ternário para a categoria já vir selecionada quando o id de uma categoria for igual a de um produto-->
                        {{ $c->nome }}
                    </option>
                @endforeach
            </select>
